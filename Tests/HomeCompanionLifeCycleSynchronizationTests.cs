...
public class HomeCompanionLifeCycleSynchronizationTests
{
    ...
    public async Task WaitForInitializationStageCompletedAsync_DoesNotSignalStageOnTimeout()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(GetRecordValue(initValuesRegistered, "CompletedAtUtc"), Is.EqualTo("<null>"));
        });
    }
    ...
    public async Task SignalInitializationStageCompletedAsync_IsIdempotent()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(GetRecordValue(initValuesRegistered, "CompletedAtUtc"), Does.Contain("2026-07-31"));
            Assert.That(GetRecordValue(initValuesRegistered, "CompletedAtUtc"), Does.Contain("11:00:00"));
        });
    }
