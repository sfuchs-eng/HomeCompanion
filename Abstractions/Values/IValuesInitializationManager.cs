...
/// 
/// The <see cref="IValuesInitializationManager"/> is to be a singleton service invoking <see cref="IValue.InitializeValue(object, HomeCompanion.Values.ValuesInitializationStage)"/> of the registered values.
...
public interface IValuesInitializationManager
{
    ...
    Task SaveValuesAsync(CancellationToken token = default);
    ...
    void RegisterInitialization(ValuesInitializationStage stage, ValuesInitializationDelegate initialization);
    void RemoveInitialization(ValuesInitializationStage stage, ValuesInitializationDelegate initialization);
}

public delegate Task ValuesInitializationDelegate(CancellationToken token = default);
